...
'''
Created on 2020/12/18  10:06

@project: OCR

@filename: gen_ocr_data2.py
...
class PreprocessResizeKeepRatioFillBG(object):
    ...
    def put_img_into_center(cls, img_large, img_small, ):
        ...
        start_width = int((width_large - width_small) / 2)
        start_height = int((height_large - height_small) / 2)
...
def args_parse():
    description = 'python gen_ocr_data.py --out_dir ./dataset --font_dir ./chinese_fonts --width 30 --height 30 --margin 4 --rotate 30 --rotate_step 1'
    # 解析输入参数
    ...
    parser.add_argument('--font_dir', dest='font_dir',
                        default='data/font/', required=False,
...
if __name__ == "__main__":
    ...
    for (char, value) in lang_chars.items():  # 外层循环是字
        ...
        for i in range(len(image_list)):
            ...
            # print(img.shape)
            if count < test_num:
                char_dir = os.path.join(test_images_dir, "%d" % value)
            else:
                char_dir = os.path.join(train_images_dir, "%d" % value)
